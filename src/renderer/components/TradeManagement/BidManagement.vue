	...
	export default {
		...
		methods: {
			...
			updateGy() {
				...
				this.$http.post(this.$api.MONTH_BIDDING_UPDATE, {
					com_id: this.$store.getters.com_id,
					month: this.month2,
					name: this.gyInput.name,
					electricity: this.gyInput.count,
					price: this.gyInput.price,
					id: this.id
				}).then(res => {
			...
			ok() {
				// this.modal1 = false;
				this.$http.post(this.$api.ADD_MOCK_BIDDING, {
					com_id: this.$store.getters.com_id,
					month: this.month2,
					name: this.bidForm.dcname,
					price: this.bidForm.baojia,
					electricity: this.bidForm.gyl
				}).then(res => {
		...
		<Row>
			<Row v-if="show==='往期竞价'">
				<Card class='height_392'>
					...
					<previous-biding-chart :comsupply="this.comsupply"
					                       :powerplant="this.powerplant"></previous-biding-chart>
				</Card>
			</Row>
			<Row gutter=15     style="height: 100%" v-if="show==='本月竞价模拟'">
				...
				<Col :lg="{span:6}" :md="{span:7}" style="height: 100%;">
				<Card class="height_392">
					...
					<div slot="extra" class="btn-group2">
						<Radio-group v-model="tabValue" type="button" v-on:on-change="switchSelected">
							<Radio label="供给侧价格"></Radio>
							<Radio label="需求侧价格"></Radio>
						</Radio-group>
					</div>
					<div class="input-container" v-if="mockSelected === '供给侧价格'">
						...
						<div class="btn-save">
							<Button type="primary" @click="updateGy" v-if='!update'>保存</Button>
							<Button type="default" disabled v-if='update'>保存</Button>
						</div>
						<div class="bidBox">
							<ul v-for="(mockData,index) in dcMockDatas">
								...
								<li class="caozuo" style="margin-left: 5px;" @click='showDelete(mockData)'>
									{{mockData.caozuo2}}
								</li>
							</ul>
						</div>
					...
					<div class="input-container" v-if="mockSelected === '需求侧价格'">
						<div class="xq-box">
							<span>最大需求量：</span>
