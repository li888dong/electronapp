			...
			saveData(){
				if(!this.isEmpty(this.addTableList)){
				   var arr = this.addTableList.yddz;
               	  this.user_nums.push(this.addTableList.user_id);
               	  var address = ''
		...
		<Card class="main-container-panel">
			...
			<h3 slot="title" style="padding-left:40px">添加售电合同</h3>
				<Form class="form-container" :label-width='130' ref='addList' :model='addList' :rules = 'addValidate'>
					...
					<div class="btn-save">
						<Button type="primary" @click='saveBtn'>保存</Button><span v-if = 'isKong' style="font-size: 12px;color:red;margin-left:5px;">内容不能为空</span>
					</div>
				    </FormItem>
			...
			<a>选择文件</a>
				    ...
				    <FormItem>
				     <div class="btn-group">
					<Button type="primary" class="mgr-20" @click='addCompact'>保存</Button>
			...
			<Modal v-model="modal1" :styles="{top:'280px'}" class="addAgreement-modal" width="850px;">
				...
				<ul class="add-msg">
					<li><label>营销用户编号:</label><Input class="width-200" placeholder="用户户号" v-model='addTableList.user_id'></Input></li>
					<li><label>用户电压等级:</label><Select class="width-200" v-model='addTableList.user_grade'><Option value="0.4kv">0.4kv</Option></Select></li>
					<li><label>报装容量:</label><Input class="width-200" v-model='addTableList.bzrl'></Input><span>KVA</span></li>
					<li><label>用电类别:</label><Select class="width-200" v-model='addTableList.ydlb'><Option value="大功业用户">大工业用户</Option></Select></li>
					<li><label>用电单元类型:</label><Select class="width-200" v-model='addTableList.yddwlb'><Option value="第一产业（农业）">第一产业（农业）</Option></Select></li>
                    <li><label>用户地址:</label><al-selector v-model='addTableList.yddz' level=2 class='address'/>
