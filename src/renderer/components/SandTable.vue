<script>
	...
	export default {
		...
		methods: {
			...
			earePowerRankM() {
				this.$http.post(this.$api.POWER_CITY_RANK, {
					com_id: this.$store.getters.com_id,
					month: this.month
				}).then(res => {
			...
			companyPowerRankM() {
				this.$http.post(this.$api.POWER_COMPANY_RANK, {
					com_id: this.$store.getters.com_id,
					month: this.month
				}).then(res => {
			...
			clientPianChaRankM() {
				this.$http.post(this.$api.CLIENT_PIANCHA_RANK, {
					com_id: this.$store.getters.com_id,
					month: this.month
				}).then(res => {
...
<template>
	<div class="main-container">
		<Row>
			<Card>
				<h3 slot="title">售电沙盘<span style="color: #ccc;padding-left: 10px;font-size: 12px;font-weight: 400;">每月度自动更新</span>
				</h3>
				<div slot="extra" class="btn-group">
					...
					<DatePicker v-if="dateType ==='月度'" type="month" placeholder="请选择月份"
					            @on-change="monthSelect"></DatePicker>
					<DatePicker v-if="dateType ==='年度'" type="year" placeholder="请选择年份"
					            @on-change="yearSelect"></DatePicker>
				</div>
				...
				<Row :gutter="10">
					...
					<div class="ranklist-container">
						...
						<ul>
							<li v-for='(item,index) in companyList1' v-bind:class={active:item.status}><span
									class="ranking">No.{{index + 1}}</span><span
									class="company-name">{{item.com_name}}</span><span class="rate">{{item.ratio * 100}}%</span>
								<span class="ranklist-bar" :style="{width:200*item.ratio+'px'}"></span></li>
							<li v-for='item in noList' v-if='bol' class='red'><span
									class="ranking">No.{{item.no}}</span><span class="company-name">{{item.num}}</span>
							</li>
						</ul>
					</div>
		...
		<Row class="mgt_15" :gutter="10">
			...
			<Card class="ranklist-container">
				...
				<ul>
					<li v-for='(item,index) in cityList'><span class="ranking">No.{{index + 1}}</span><span
							class="city">{{item.name}}</span>
						<span class="ranklist-bar" :style="{width:200*(item.num||item.bndyjdl/total1)+'px'}"></span>
					</li>
			...
			<Card class="ranklist-container">
				...
				<ul>
					<li v-for='(item,index) in companyList2' :title='item.name'><span
							class="ranking">No.{{index + 1}}</span><span class="company-name">{{item.name}}</span>
			...
			<Card class="ranklist-container">
				...
				<ul>
					<li v-for='(item,index) in companyList3' :title='item.name'><span
							class="ranking">No.{{index + 1}}</span><span class="company-name">{{item.name}}</span><span
							class="rate">{{item.ratio * 100}}%</span>
						<span class="ranklist-bar" :style="{width:200*item.ratio+'px'}"></span></li>
				</ul>
