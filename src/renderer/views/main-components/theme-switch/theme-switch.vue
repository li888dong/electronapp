...
import config from '../../../../../.electron-vue/config.js';
export default {
    name: 'themeSwitch',
    data () {
        return {
    ...
    methods: {
        setTheme (themeFile) {
            ...
            if (config.env.indexOf('dev') > -1) {
                stylePath = './src/renderer/views/main-components/theme-switch/theme/';
            } else {
                stylePath = 'dist/';
            }
            if (mainTheme !== 'b') {
    ...
    created () {
        let path = '';
        if (config.env.indexOf('dev') > -1) {
            path = './src/renderer/views/main-components/theme-switch/theme/';
        } else {
            path = 'dist/';
        }
